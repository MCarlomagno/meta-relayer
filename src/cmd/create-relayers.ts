...
export async function createRelayers() {
	...
	// create relayers
	const relayersWithApiKeys = await Promise.all(relayerNames.map((name) => createRelayer(defennder, name, network)));

	console.log('Relayers created', relayersWithApiKeys);
}

async function createRelayer(client: Defender, name: string, network: string): Promise<RelayerApiKey> {
	return client.relay.create({
		network,
		minBalance: '0',
		name,
